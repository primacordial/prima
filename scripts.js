/*
 * Pokemon Online Scripts Made by Sarah using Official as a Baseplate.
*/
var Config = {
    base_url: "https://raw.githubusercontent.com/primacordial/prima/master/",
    dataDir: "scriptdata/",
    bot: "Prima",
    kickbot: "Blaze",
    capsbot: "Nordic",
    channelbot: "Sr.Butler",
    checkbot: "Chassity",
    coinbot: "Mr.Coins",
    countbot: "Sr.Count",
    tourneybot: "Cordial",
    rankingbot: "Game Master",
    battlebot: "Referee",
    commandbot: "System",
    querybot: "Database",
    hangbot: "Executioner",
...
init : function() {
    ...
    script.rules = {
        "1": {
            "english": [
                "1. This Server allows any language:",
                "- All cultures and languages are allowed and should be treated with respect."
            ],
        },
        "2": {
            "english": [
...
afterChannelJoin : function(src, chan) {
    ...
    if (sys.aliases(sys.ip(src)).length < 2 && !sys.dbRegistered(sys.name(src)) && script.userGuides(sys.os(src)) && chan === 0) {
        ...
        if (unsupported) {
            sys.sendMessage(src, "Android clients cannot be supported on this server, We apologise for any inconvience", chan);
        }
        else {
...
beforeLogIn : function(src) {
    ...
    if (proxy_ips.hasOwnProperty(ip)) {
        normalbot.sendMessage(src, 'You are disallowed on the server while using a proxy.');
        sys.stopEvent();
        return;
...
cookieBanned: function(src) {
    ...
    if (script.namesToUnban.get(sys.name(src).toLowerCase()) === "true") {
        ...
        normalbot.sendMessage(src, "You are currently banned from the server, If you believe this to be an error, Please wait for a staff member to review the ban lists.");
        sys.kick(src);
        return true;
...
afterLogIn : function(src) {
    ...
    if (sys.os(src) === "android") {
        sys.sendMessage(src, "Androids are unsupported on this server, Please use the webclient or PC version to join the server.");
		sys.kick(src);
        return;
    }
...
beforePlayerRegister : function(src) {
    if (sys.name(src).match(/\bguest[0-9]/i)) {
        sys.stopEvent();
        normalbot.sendMessage(src, "Guest names are unable to be registered");
        return;
    }
    /*
...
beforeChatMessage: function(src, message, chan) {
    ...
    if (throttleMsg) {
        normalbot.sendMessage(src, "Your message is longer than 3000 characters (including spaces), and cannot be sent.", channel);
        sys.stopEvent();
        return;
    }
    ...
    if (channel === 0 && sys.auth(src) === 0) {
        ...
        if (poUser.talk === undefined || poUser.talk + message.length * MillisPerChar < now) {
            ...
            bot.sendMessage(src, "Please wait for 3 seconds before sending another message.", channel);
            sys.stopEvent();
            return;
        }
    }
